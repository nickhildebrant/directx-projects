...
// Node
Node::Node( const std::string& name, std::vector<Mesh*> meshPtrs, const DirectX::XMMATRIX& trans ) noexcept : meshPtrs( std::move( meshPtrs ) ), name( name )
{
...
void Node::ShowTree( int& nodeIndexTracked, std::optional<int>& selectedIndex, Node*& pSelectedNode ) const noexcept
{
	...
	// render this node
	const bool expanded = ImGui::TreeNodeEx( (void*) (intptr_t) currentNodeIndex, node_flags, name.c_str() );
